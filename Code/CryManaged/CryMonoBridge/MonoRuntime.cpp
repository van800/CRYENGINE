...
bool CMonoRuntime::InitializeRuntime()
{
	...
	const char * mono_env_options = getenv("MONO_ENV_OPTIONS");
	if (mono_env_options != nullptr)
	{
		// Usually MONO_ENV_OPTIONS looks like: --profile=<profiler options>
		const char * prefix = "--profile=";
		if (const char * prof_options = std::strstr(mono_env_options, prefix))
			MonoInternals::mono_profiler_load(prof_options + std::strlen(prefix));
	}
